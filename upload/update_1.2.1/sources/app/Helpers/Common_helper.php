    ...
    function count_category_courses($category_id)
    {
        ...
        if ($category) {
            if ($category->parent_id == 0) {
                ...
                $courses    = App\Models\Course::where('status', 'active')->whereIn('category_id', $categories)->orWhere('category_id', $category_id)->count();
            } else {
                $courses = App\Models\Course::where('status', 'active')->where('category_id', $category_id)->count();
            }
            return ($courses > 1) ? "{$courses} " . get_phrase('courses') : "{$courses} " . get_phrase('course');
        } else {
...
if (!function_exists('is_purchased_bootcamp')) {
    function is_purchased_bootcamp($bootcamp_id, $user_id = null)
    {
        ...
        $purchase = App\Models\BootcampPurchase::where('user_id', $user_id)->where('bootcamp_id', $bootcamp_id)->firstOrNew();
        return $purchase;
    }
}
